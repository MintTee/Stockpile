...
`rednet.send(stockpile_server_id, {"command", [command_UUID]}, "protocol")`

Examples :

`rednet.send(123, {[[scan(inventories.storage)]], math.random(1, 2^32)}, "stockpile")`
`rednet.send(321, [[list_all_inventories()]], "stockpile")`
...
**Returns**
1. table - {total_slots = number, used_slots = number}
